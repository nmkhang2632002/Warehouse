...
function fnShowMyWarehouseVisualizerDemo() {
  ...
  if (
    ...
    fetchDataVisualize("layout").then((layoutData) => {
      var warehouseScene = { scene: fnBuildWarehouse(layoutData) };
      if (warehouseScene) {
        fetchDataVisualize("inventory").then((inventoryData) => {
          fnShowWarehouse(
            warehouseName,
            warehouseScene,
            inventoryData,
  ...
  async function fetchDataVisualize(option) {
    try {
      const res = await fetch(
        `https://7bf7-2402-800-637c-259e-215b-58c4-28ef-c556.ngrok-free.app/warehouse/${option}`,
        {
          headers: {
