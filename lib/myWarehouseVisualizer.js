...
var pstyle =
  "background-color: #F5F6F7; border: 1px solid #dfdfdf; padding: 5px;"; //Panel styling
...
var appWarehouses = [
  { warehouseName: "Upload..." },
  { warehouseName: "CSV URLs..." },
]; //Array of objects for demo and uploaded visuals
        ...
        toolbar: {
          items: [
            {
              ...
              text: function (item) {
                return (
                  "Warehouse: " +
                  (item.selected <= 0 ? "" : item.items[item.selected].text)
                );
              },
            },
            { type: "break" },
          ...
          onClick: function (event) {
            event.done(function () {
              ...
              if (
                aTargets.length == 1 &&
                ["upload", "toggleVisualGrid", "toggleAnalyzer"].indexOf(
                  aTargets[0]
                ) == -1
              )
              ...
              switch (aTargets[0]) {
                case "warehouse": //Find programmatically with: w2ui.toolbar.items.find(function(item) {return item.id == "warehouse"}).selected
                  ...
                  if (vDropDown.selected <= 1) {
                    ...
                    vDropDown.lastSelected = selected;
                    vDropDown.text =
                      "Warehouse: " + vDropDown.items[selected].text;
                ...
                case "toggleVisualGrid":
                  ...
                  if (mainPanel.get("preview").mdvPreHideSize) {
                    mainPanel.sizeTo(
                      "preview",
                      mainPanel.get("preview").mdvPreHideSize
                    ); //Must operate on preview, not main (bug)
                    mainPanel.sizeTo(
                      "left",
                      mainPanel.get("left").mdvPreHideSize
                    );
                    ...
                    mainPanel.get("preview").mdvPreHideSize =
                      mainPanel.get("preview").size;
                    mainPanel.get("left").mdvPreHideSize =
                      mainPanel.get("left").size;
                ...
                case "toggleAnalyzer":
                  ...
                  if (!mainPanel.get("left").hidden)
                    //Handling final sizing of preview
                    mainPanel.get("left").mdvPreHideSize =
                      mainPanel.get("left").size;
                ...
                case "Help":
                  switch (aTargets[1]) {
                    ...
                    case "w2ui":
                      var links = {
                        documentation:
                          "https://github.com/MarioDelgadoSr/MyWarehouseVisualizerDoc#my-warehouse-visualizer-documentation",
              ...
              function fnPopUp(appContainer, title, html, wf, hf) {
                var width =
                  $(appContainer).width() == 0
                    ? appContainer.parentElement.clientWidth
                    : $(appContainer).width();
                var height =
                  $(appContainer).height() == 0
                    ? appContainer.parentElement.clientHeight
                    : $(appContainer).height();
                ...
                w2popup.open({
                  ...
                  body: html,
                  buttons:
                    '<button onclick="w2popup.close();">' +
                    translate("PopUpClose") +
                    "</button>",
                  showMax: false,
                  showClose: true,
                  width: width / wf,
...
function fnShowMyWarehouseVisualizerDemo() {
  ...
  var warehouseIndex = parentLayout
    .get("main")
    .toolbar.items.find(function (item) {
      return item.id == "warehouse";
    }).selected;
  ...
  if (!appWarehouses[warehouseIndex].layoutData) {
    fetchDataVisualize("layout", warehouseName).then((layoutData) => {
      var warehouseScene = { scene: fnBuildWarehouse(layoutData) };
      if (warehouseScene) {
        fetchDataVisualize("inventory", warehouseName).then((inventoryData) => {
          fnShowWarehouse(
            warehouseName,
            warehouseScene,
            inventoryData,
    ...
    var warehouseScene = {
      scene: fnBuildWarehouse(appWarehouses[warehouseIndex].layoutData),
    };
    if (warehouseScene) {
      fnShowWarehouse(
        warehouseName,
        warehouseScene,
        appWarehouses[warehouseIndex].inventoryData,
        appWarehouses[warehouseIndex].layoutData
      );
    }
  }
  ...
  async function fetchDataVisualize(option, size) {
    try {
      const res = await fetch(
        `https://7bf7-2402-800-637c-259e-215b-58c4-28ef-c556.ngrok-free.app/warehouse/${option}?size=${size}`,
        {
          headers: {
  ...
  function fnShowWarehouse(
    ...
    if (inventoryData) {
      ...
      warehouseVisual.joinDataToVisual(
        inventoryData,
        warehouseScene,
        dataKey,
        visualKey
      ); //warehouseVisual.joinDataToVisual(gltfDataFromFile);  //testing with no data

      //Pallets are created opaque...they're dependent on actual inventory.  Join takes care of that logic.
      warehouseVisual.join.forEach((join) => {
        join.visualObj.children[1].material.opacity = 1;
      }); //children[1] == pallet
    } //if
    ...
    var container =
      typeof parentLayout != "undefined"
        ? parentLayout //Container can be a div or layout.  If layout, app placed in 'main' panel
        : $(document.createElement("div")).appendTo(
            $(document.body).css({ margin: "0px" })
          ); //Layout Height Design Pattern: https://github.com/vitmalina/w2ui/issues/105

    appLayout = new applicationLayout();
    appLayout.display(container, warehouseVisual);

    var vDropDown = parentLayout
      .get("main")
      .toolbar.items.find(function (item) {
        return item.id == "warehouse";
      });
  ...
  function fnBuildWarehouse(layoutData) {
    ...
    for (var i = 0; i < layoutData.length; i++) {
      /* https://threejs.org/docs/index.html#api/en/geometries/BoxBufferGeometry
				...
				*/
      var geometryKey =
        (layoutData[i]["WIDTH"] ?? layoutData[i]["width"]) +
        ":" +
        (layoutData[i]["HEIGHT"] ?? layoutData[i]["height"]) +
        ":" +
        (layoutData[i]["DEPTH"] ?? layoutData[i["depth"]]);
      ...
      slot.position.x =
        layoutData[i]["X"] ??
        layoutData[i]["x"] +
          (layoutData[i]["HEIGHT"] ?? layoutData[i]["height"]) / 2;
      slot.position.y =
        layoutData[i]["Z"] ??
        layoutData[i]["z"] +
          (layoutData[i]["DEPTH"] ?? layoutData[i]["depth"]) / 2;
      slot.position.z =
        layoutData[i]["Y"] ??
        layoutData[i]["y"] +
          (layoutData[i]["HEIGHT"] ?? layoutData[i]["height"]) / 2;
      slot.userData.aisle = layoutData[i]["AISLE"] ?? layoutData[i]["aisle"];
      slot.userData.aisleSide =
        layoutData[i]["AISLESIDE"] ?? layoutData[i]["aisleside"];
      slot.userData.centerAxis =
        layoutData[i]["CENTERAXIS"] ?? layoutData[i]["centeraxis"];
      slot.userData.bay = layoutData[i]["BAY"] ?? layoutData[i]["bay"];
...
function fnGetCsvUrls() {
  w2popup.load({
    ...
    buttons:
      '<button class="w2ui-btn" onclick="fnHandleCsvUrls()">OK</button> <button class="w2ui-btn" onclick="w2popup.close()">Cancel</button>',
  });
} //fnGetCsvUrls
...
function fnHandleFileUploads(event) {
  ...
  var csvLayoutFile =
    files[0].name.toLowerCase().indexOf("layout") != -1
      ? files[0]
      : files[1].name.toLowerCase().indexOf("layout") != -1
      ? files[1]
      : null;

  var csvInventoryFile =
    files[0] == csvLayoutFile
      ? files[1].name.toLowerCase().indexOf("inventory") != -1
        ? files[1]
        : null
      : files[0].name.toLowerCase().indexOf("inventory") != -1
      ? files[0]
      : null;

  if (!csvLayoutFile && !csvInventoryFile) {
    w2alert(
      "Files must have 'Inventory' and 'Layout' in there names to distinguish them."
    );
  ...
  layoutReader.onloadend = function () {
    ...
    inventoryReader.onloadend = function () {
      ...
      parentLayout
        .get("main")
        .toolbar.get("warehouse")
        .items.push({
          id: appWarehouses.length - 1,
          text: csvLayoutFile.name.split(".csv")[0],
          checked: true,
        });
      parentLayout.get("main").toolbar.get("warehouse").selected =
        appWarehouses.length - 1;
...
function fnSetPredefinedScales() {
  ...
  fnBuildPredefinedScales(mdvScales, {
  ...
  timeOutVar = window.setInterval(function () {
    ...
    scaleIndex =
      scaleIndex + (scaleDirection % 2) > maxScaleIndex
        ? (scaleDirection++, 0)
        : scaleIndex + (scaleDirection++ % 2);
  }, 2000);
...
function applicationLayout() {
  ...
  app.destroy = function () {
    ...
    Object.keys(w2ui).forEach(function (objKey) {
      //Remove all w2ui instances for this object
      if (objKey.indexOf(app.dataVisual.warehouseName + "_") != -1)
        w2ui[objKey].destroy();
    });
...
function myDataVisualizer() {
  ...
  mdv.display = function (mdvLayout, dataVisual) {
    ...
    scaleControlLayout.onRender = function (event) {
      event.done(function (event) {
        fnFinshLayout();
      });
    }; //onRender
    ...
    function fnFinshLayout() {
      ...
      mdv.visualizer = fnBuildDataVisualizer(
        mdvLayout.content("preview"),
        dataVisual
      ); //Warehouse Scene in visualizer object
      ...
      if (dataVisual.data.length == 0) {
        ...
        dataVisual.activeGridColumn = {
          grid: dataVisual.dataGrid,
          column: dataVisual.dataKey,
        }; //First column to be visualized
        ...
        // Refresh Handler
        fnRefreshGrid = function (event) {
          //Refresh grids on

          for (var i = 0; i < mdv.visualGrid.columns.length; i++) {
            //Clear column selection
            mdv.visualGrid.columns[i].style = "";
          } //for

          for (var i = 0; i < mdv.dataGrid.columns.length; i++) {
            //Clear column selection
      ...
      // Resize Handler
      fnResizeMdvLayout = function (event) {
        //Resize visualizer on layout's resize
        event.done(function (event) {
          //http://w2ui.com/web/docs/1.5/utils/events
    ...
    function fnSearchIntersects(intersects, bSearch) {
      if (intersects == "reset") {
        dataVisual.layoutData.forEach(function (layoutRow) {
          layoutRow.visualObj.visible = true;
        });
        ...
        if (
          slotRow &&
          slotRow[dataVisual.dataKey] == dataVisual.visualGrid.last.search &&
          bSearch
        ) {
          //Toggle isolation searching
        ...
        if (slotRow && bSearch) {
          ...
          setTimeout(function () {
            dataVisual.visualGrid.searchReset(false); //Reset search (trigger search logic), but don't refresh the grid http://w2ui.com/web/docs/1.5/w2grid.searchReset
            dataVisual.visualGrid.search(
              dataVisual.dataKey,
              slotRow[dataVisual.dataKey]
      ...
      function fnFindDataRow(intersects) {
        ...
        for (var i = 0; i < intersects.length; i++) {
          var data =
            dataVisual.activeGridColumn.grid.name.indexOf("visualGrid") != -1
              ? dataVisual.layoutData
              : dataVisual.join;

          var bFoundMatch = data.some(function (row) {
            if (isSlotObj(row.visualObj, intersects[i].object)) {
              slotRow =
                dataVisual.activeGridColumn.grid.name.indexOf("visualGrid") !=
                -1
                  ? row
                  : row.dataRow;
              return true;
    ...
    function fnBuildDataVisualizer(container, dataVisual) {
      ...
      function dataVisualizer(overrideProperties) {
        var properties = {
          //Default warehouse layout props; attributes can be changed with attributes in ojb3dProperties
          version: 1.1,
          ...
          axesHelper: { obj: null, visible: false },
          gridHelper: {
            obj: null,
            gridSize: null,
        ...
        function fnGetResizeCallback() {
          ...
          var callback = function () {
            ...
            var containerWidthHeight = fnGetContainerWidthHeight(
            ...
            properties.renderer.obj.setSize(
              properties.containerWidth,
              properties.containerHeight
            );

            // update the camera
            properties.camera.obj.aspect =
              properties.containerWidth / properties.containerHeight;
        ...
        function fnDisplay(container) {
          ...
          properties.renderer.obj.render(
            properties.scene.obj,
            properties.camera.obj
          );
        ...
        function fnBuildPerformanceStats(properties) {
          ...
          for (
            var i = 0;
            i < properties.stats.domElement.children.length;
            i++
          ) {
            properties.stats.domElement.children[i].style.display = "";
          }
        ...
        function fnDestroy() {
          ...
          if (typeof properties.fnHandleKeyMouse != "undefined") {
            ...
            properties.renderer.obj.domElement.removeEventListener(
              "mousedown",
              properties.fnHandleKeyMouse
            );
            properties.renderer.obj.domElement.removeEventListener(
          ...
          if (properties.animationFrame)
            window.cancelAnimationFrame(properties.animationFrame);
          if (properties.scene.obj) {
            fnClear(properties.scene.obj);
            fnDispose(properties.scene.obj);
          }
          if (properties.renderer.obj) {
            properties.container.removeChild(
              properties.renderer.obj.domElement
            );
        ...
        function fnSetSceneProperties(overrideProperties) {
          properties.warehouseData = overrideProperties.warehouseData
            ? overrideProperties.warehouseData
            : properties.warehouseData; //Use override data/schema if they exists.
          properties.schema = overrideProperties.schema
            ? overrideProperties.schema
            : properties.schema;
        ...
        function fnRotate() {
          //rotate clockwize
          properties.camera.obj.position.applyAxisAngle(
            new THREE.Vector3(0, 1, 0),
            -Math.PI / 2
          ); //rotate clockwize 90 degrees
          properties.controls.obj.update();
        }
        ...
        function fnSetBirdsEyeView() {
          ...
          properties.camera.obj.position.set(
            0,
            properties.gridHelper.gridSize,
            properties.gridHelper.gridSize
          );
        ...
        function fnBuildGridHelper(settings) {
          ...
          properties.gridHelper.obj = new THREE.GridHelper(
            settings.size,
            settings.divisions,
            settings.colorCenter,
            settings.colorGrid
          );
          properties.gridHelper.obj.position.set(
            settings.x,
            settings.y,
            settings.z
          );
        ...
        function fnBuildScene() {
          ...
          properties.scene.obj.background = new THREE.Color(
            properties.scene.background
          );
          ...
          properties.boundingBox.obj = new THREE.BoxHelper(
            properties.visualGroup,
            properties.boundingBox.color
          ); //Used to get the group's coordinates
          ...
          properties.camera.obj = new THREE.PerspectiveCamera(
            fieldOfView,
            aspectRatio,
            perspectiveNear,
            perspectiveFar
          ...
          // RENDERER
          if (Detector.webgl)
            properties.renderer.obj = new THREE.WebGLRenderer({
              antialias: true,
            });
          else properties.renderer.obj = new THREE.CanvasRenderer();
          ...
          var zIndex = parseFloat($(container).css("z-index"))
            ? parseFloat($(properties.container).css("z-index"))
            : 0;
          ...
          // ORBIT CONTROLS
          properties.controls.obj = new THREE.OrbitControls(
            properties.camera.obj,
            properties.renderer.obj.domElement
          );
          ...
          properties.controls.obj.maxPolarAngle =
            properties.controls.maxPolarAngle * (Math.PI / 180);
          ...
          // AXESHELPER
          properties.axesHelper.obj = new THREE.AxesHelper(
            properties.gridHelper.gridSize
          ); //Add x,y,z axis helper at scenes 0,0,0
          properties.axesHelper.obj.position.set(
            -properties.boundingBox.boundingRange.x,
            -properties.boundingBox.boundingRange.y,
            -properties.boundingBox.boundingRange.z
          );
          ...
          properties.controls.obj.maxDistance =
            properties.gridHelper.gridSize * 1.5; //Don't allow orbital control to go out beyond the 150% of gridSize
          ...
          properties.fnHandleKeyMouse = function (event) {
            ...
            switch (
              event.type //Based on document, so offset is relative to doucument if the intent is to use event.offsetX/offsetY
            ) {
              case "keydown":
                if (!event.ctrlKey) {
              ...
              case "dblclick":
                ...
                if (intersects) {
                  var dataRow = properties.fnSearchIntersects(
                    intersects,
                    !event.ctrlKey
                  );
                  ...
                  if (event.ctrlKey) {
                    ...
                    if (
                      dataVisual.selectionLinks.length > 0 ||
                      !$.isEmptyObject(
                        dataVisual.getJoinByKey(dataRow[dataVisual.dataKey])
                          .selectionLink
                      )
                    ) {
                      var selectName =
                        dataVisual.warehouseName +
                        "_links_" +
                        dataRow[dataVisual.dataKey];

                      $("[name='" + selectName + "']")
                        .clone()
                        .appendTo(properties.toolTipDiv);
                    } //if
                  } //if
            ...
            function fnHandleMouseMove(toolTip) {
              ...
              if (intersects) {
                ...
                if (dataRow) {
                  ...
                  var columnText0 =
                    column == "recid" || column == dataVisual.dataKey
                      ? ""
                      : colorColumn.trim() + " ";
                  var text = dataVisual.activeGridColumn.grid.columns.find(
                    function (col) {
                  ...
                  var columnText =
                    ...
                    dataVisual.data[0][dataVisual.colorPrefix + column] !=
                      undefined
                      ? columnText0 + text
                      : text;
                  var formattedValue =
                    dataVisual.dataTypes[column].type != "text"
                      ? fnFloatFormat(
                          dataVisual,
                          dataVisual.activeGridColumn.column,
                          dataRow[dataVisual.activeGridColumn.column]
                        )
                      : dataRow[dataVisual.activeGridColumn.column];
                  var text =
                    column == dataVisual.dataKey ||
                    column == dataVisual.visualKey
                      ? dataRow[dataVisual.dataKey]
                      : dataRow[dataVisual.dataKey] +
                        ": " +
                        columnText +
              ...
              function fnShowToolTip(text, latestMouseProjection) {
                if (toolTip && latestMouseProjection) {
                  ...
                  var tooltipPosition = latestMouseProjection
                    .clone()
                    .project(camera);

                  //Relative to the renderer
                  tooltipPosition.x =
                    tooltipPosition.x * canvasHalfWidth + canvasHalfWidth;
                  tooltipPosition.y =
                    -(tooltipPosition.y * canvasHalfHeight) + canvasHalfHeight;
          ...
          properties.renderer.obj.domElement.addEventListener(
            "mousemove",
            properties.fnHandleKeyMouse,
            false
          );
          ...
          function fnGetRayCasterIntersects(properties) {
            ...
            properties.mouse.x =
              (properties.offsetX / renderer.domElement.clientWidth) * 2 - 1; //Normaized and relative to container that my be offset within the client
            properties.mouse.y =
              -(properties.offsetY / renderer.domElement.clientHeight) * 2 + 1;
            ...
            var intersects = properties.raycaster.intersectObjects(
              dataVisual.layoutData.map(function (layoutRow) {
                return layoutRow.visualObj;
              }),
              true
            );
        ...
        function fnBuildVisual() {
          ...
          visualGroup.traverse(function (node) {
            //object may be a node deep in the mesh hiearchy
            if (node.userData) {
              if (node.userData[properties.visualizeProp]) {
                //GLTF file would have associated with mesh:  {Blender: ...."extras": {"visualize": 1.0} ...}, In Blender it's custom property: https://docs.blender.org/manual/en/latest/data_system/custom_properties.html
          ...
          function fnSaveChildrenOriginalColor(node) {
            //parent and children (if any) have userData.originalColor assigned to them as well
            node.traverse(function (node) {
              if (node.material)
                node.userData.originalColor = new THREE.Color().copy(
                  node.material.color
                ); //copy the original color assoicated with the mesh
            });
    ...
    function fnGetDataTypes(dataVisual, dataSource) {
      var visualizeData =
        dataSource == "inventory" ? dataVisual.data : dataVisual.layoutData;
        ...
        aColumns.forEach(function (column) {
          if (dataTypes[column]) {
            ...
            dataTypes[column].type =
              dataTypes[column].type == "text"
                ? "text"
                : typeof mergedRow[column] == "string"
                ? "text"
                : "float"; //For warehouse, using d3.autoType
          } //if
          else {
            //dataTypes[column] =  {type: isNaN(Number(mergedRow[column])) ? "text" : "float"};
            dataTypes[column] = {
              type: typeof mergedRow[column] == "string" ? "text" : "float",
            };
          } //else
        }); //aColumns.forEach
      } //for
      ...
      var aColumns = Object.keys(visualizeData[0]).filter((key) => {
        return dataSource == "inventory" ? true : key != "visualObj";
      }); //Filter out visualObj for layoutData

      aColumns.forEach(function (column) {
        dataTypes[column].colorColumn =
          dataTypes[dataVisual.colorPrefix + column]; //May be undefined if no color if predefined for column
        ...
        dataTypes[column].type =
          column.substring(0, dataVisual.urlPrefix.length) ==
          dataVisual.urlPrefix
            ? "url"
            : column.substring(0, dataVisual.datePrefix.length) ==
              dataVisual.datePrefix
            ? "date"
            : column.substring(0, dataVisual.dateTimePrefix.length) ==
              dataVisual.dateTimePrefix
            ? "dateTime"
            : column.substring(0, dataVisual.timePrefix.length) ==
              dataVisual.timePrefix
            ? "time"
            : dataTypes[column].type;
        ...
        dataTypes[column].key = dataTypes[column].isVisualData
        ...
        if (dataTypes[column].type == "float") {
          ...
          for (var i = 0; i < visualizeData.length; i++) {
            dataTypes[column].numDecimals = Math.max(
              dataTypes[column].numDecimals,
              fnDecimaPlaces(visualizeData[i][column])
            );
          } //for
        } //if
        ...
        switch (true) {
          case dataTypes[column].type == "date":
            dataTypes[column].caption = column.substring(
              dataVisual.datePrefix.length
            );
            break;
          case dataTypes[column].type == "dateTime":
            dataTypes[column].caption = column.substring(
              dataVisual.dateTimePrefix.length
            );
            break;
          case dataTypes[column].type == "time":
            dataTypes[column].caption = column.substring(
              dataVisual.timePrefix.length
            );
            break;
          default:
      ...
      aColumns.forEach(function (column) {
        ...
        dataTypes[column].aUniqueCategories = Array.from(
          new Set(
            visualizeData.map(function (row) {
          ...
          //https://stackoverflow.com/questions/4373018/sort-array-of-numeric-alphabetical-elements-natural-sort
          var a1 = typeof a,
            b1 = typeof b;
    ...
    function fnBuildGrid(dataVisual, visualizer, gridType) {
      ...
      function fnGetGridProperties(dataVisual, visualizer, gridType) {
        var gridRecords =
          gridType == "visual" ? dataVisual.layoutData : dataVisual.data;
        ...
        var filterTest = function (field) {
          return field == "recid"
            ? true
            : gridType == "visual"
            ? dataTypes[field].isVisualData
            : !dataTypes[field].isVisualData && dataTypes[field].type != "url";
        }; //filterTest

        var dataFields = Object.keys(
          gridType == "visual" ? dataVisual.layoutData[0] : dataVisual.data[0]
        ).filter(function (field) {
          return filterTest(
            field.split(dataVisual.colorPrefix)[
              field.split(dataVisual.colorPrefix).length - 1
            ]
          );
        });
        var columns = [];
        ...
        if (dataTypes[dataVisual.colorPrefix + key]) {
          //position here for column grouping

          columns.push({
            field: dataVisual.colorPrefix + key,
        ...
        dataVisual.join.forEach(
          (join) =>
            (hasSelection = hasSelection
              ? true
              : !$.isEmptyObject(join.selectionLink))
        );

        if (
          gridType == "data" &&
          (dataVisual.selectionLinks.length > 0 || hasSelection)
        ) {
          columns.push(
            {
              field: "_links",
              ...
              render: function (record, rowIndex, columnIndex) {
                if (
                  dataVisual.selectionLinks.length == 0 &&
                  $.isEmptyObject(
                    dataVisual.getJoinByKey(record[dataVisual.dataKey])
                      .selectionLink
                  )
                )
                ...
                select += " onchange='";
                select +=
                  'if (this.value == -1) return;  window.open(this.value , "_blank");' +
                  "'>";
                ...
                if (dataVisual.selectionLinks.length > 0) {
                  dataVisual.selectionLinks.forEach(function (selection, i) {
                    ...
                    Object.keys(selection).forEach(function (parm, i) {
                      if (parm != "url" && parm != "urlText") {
                        linkParms.push(
                          dataVisual.data[record.recid - 1][selection[parm]]
                        ); //Booyah Booyah!
                        urlParms +=
                          parm +
                          "=" +
                  ...
                  //record level selection link

                  var selection = dataVisual.getJoinByKey(
                    record[dataVisual.dataKey]
                  ).selectionLink;
        ...
        if (bColorField) {
          ...
          for (var i = 1; i < columns.length - 1; i++) {
            //skip over recid and last columm ( i < columns.length - 1)

            if (!fnIsPresetColor(columns[i].field) && !columns[i].hidden) {
              if (dataTypes[dataVisual.colorPrefix + columns[i].field]) {
                //If the current field as a preset color associate with it
          ...
          if (columns[columns.length - 1].field == "_links") {
            //links is hidden and was handled above
        ...
        return {
          ...
          sortData: columns
            .filter(function (row) {
              return row.field != "_links";
            })
            .map(function (row) {
              return { field: row.field, direction: "ASC" };
            }),
          ...
          searches: columns
            ...
            .map(function (row, i, arr) {
              return {
                field: row.field,
                caption: fnIsPresetColor(row.field)
                  ? arr[i - 1].field.trim() + " COLOR"
                  : row.field.trim() == "recid"
                  ? "Join Row"
          ...
          onSelect: function (event) {
            //Added with when multiSelect:true was enabled
          ...
          onSearch: function (event) {
            // http://w2ui.com/web/docs/1.5/w2grid.onSearch
        ...
        function fnGridColumnRender(record, row_index, column_index) {
          ...
          var fieldText =
            fieldColorColumn == "recid"
              ? record.recid
              : record[fieldColorColumn];
          var predDefinedColor = fnIsPresetColor(fieldColorColumn);
          var fieldValueColumn = predDefinedColor
            ? this.columns[column_index - 1].field
            : this.columns[column_index].field; //For predefined colors columns, reference the column to its left
          var fieldValueType = dataVisual.dataTypes[fieldValueColumn].type;

          if (!this.toolbar.get("styleGrid").checked)
            //No visualization formatting
            return predDefinedColor
              ? fieldText
              : fieldValueType != "text"
          ...
          if (fieldValueType != "text") {
            var columnAttributes = fnGetColumnAttributes(
              fieldValueColumn,
              dataVisual,
              gridRecords
            );
    ...
    function fnGetGridToolbar(dataVisual, visualizer, container) {
      function fnBuildRadioIds(prefix, start, end, increment) {
        ...
        for (var i = start; i <= end; i = i + increment) {
          arr.push({
            id: prefix + "_" + (increment < 1 ? String(i).substring(0, 3) : i),
            text: increment < 1 ? String(i).substring(0, 3) : String(i),
          });
        } //for
        return arr;
      } //fnBuildRadioIds
      ...
      var aZoomSpeed = fnBuildRadioIds("zp", 0.1, 2.1, 0.1);
      var aPanSpeed = fnBuildRadioIds("ps", 0.1, 2.1, 0.1);
        ...
        case "dataGrid":
          return {
            ...
            onClick: function (event) {
              switch (event.target) {
                case "downloadDataButton":
                  if (
                    ...
                    var gridData =
                      oGrid.searchData.length > 0
                        ? fnFilterDownLoadData(
                            oGrid.last.searchIds.map(function (searchId) {
                    ...
                    var mdvDownLoadData =
                      ...
                      encodeURIComponent(d3.csvFormat(gridData)); //https://github.com/d3/d3-dsv#csvFormat
                    ...
                    if (anchor.length == 0) {
                      var anchor = $(document.createElement("a")).attr(
                        "download",
                        dataVisual.warehouseName +
                          (dataVisual.warehouseName.indexOf(".csv") == -1
                            ? ".csv"
                            : "")
                      );
                  ...
                  function fnFilterDownLoadData(data) {
                    var columns = Object.keys(data[0]).filter(
                      (column) =>
                        column == dataVisual.dataKey ||
                        !dataVisual.dataTypes[column].isVisualData
                    );
                    ...
                    data.forEach((row, i) => {
                      ...
                      columns.forEach((column) => {
                        downLoadData[i][column] = row[column];
                      });
                    });
              ...
              event.done(function (event) {
                if (event.target == "styleGrid") {
                  //Button to Toggle Visualization Styling of Data Grid Selected
          ...
          return {
            items: [
              ...
              },
              {
                type: "color",
                id: "bBoxColor",
                color: visualizer.boundingBox.obj.material.color.getHexString(),
                text: "Color",
                tooltip: "Set Bounding Box Color",
              },
              { type: "break" },
              {
                type: "menu-radio",
              ...
              },
              {
                ...
                color: new THREE.Color(
                  visualizer.gridHelper.colorGrid
                ).getHexString(),
                text: "Grid Color",
                tooltip: "Set Grid Color",
              },
              {
                type: "color",
                id: "colorCenter",
                color: new THREE.Color(
                  visualizer.gridHelper.colorCenter
                ).getHexString(),
                text: "Center Line Color",
                tooltip: "Set Grid Center Line Color",
              },
              {
                type: "menu-radio",
                id: "divisions",
            ...
            onClick: function (event) {
              ...
              switch (event.target) {
                ...
                case "gpuPerformance":
                  ...
                  $("#gpuContainer").css(
                    "display",
                    isVisible ? "none" : "inline"
                  );

                  if ($("#gpuContainer").contents().length == 0) {
                    $("#gpuContainer").append(
                      $(visualizer.stats.domElement).contents()
                    );
                  } //

                case "bBoxColor":
                  visualizer.boundingBox.obj.material.color.set(
                    this.get("bBoxColor").color
                  );
                  break;
              ...
              event.done(function (event) {
                switch (event.target.split(":")[0]) {
                  case "axesHelper":
                    visualizer.axesHelper.obj.visible =
                      this.get("axesHelper").checked;
                    break;
                  case "background":
                    visualizer.scene.obj.background.set(
                      this.get("background").color
                    );
                    edgeMaterial.color = fnGetBackGroundColorInvert(
                      this.get("background").color
                    ); //edgeMaterial is global
                    break;
                  case "boundingBox":
                    visualizer.boundingBox.obj.visible =
                      this.get("boundingBox").checked;
                    break;
                  case "fov":
                    visualizer.camera.fov = parseInt(
                      this.get("fov").selected.split("_")[1]
                    );
                    visualizer.camera.obj.fov = visualizer.camera.fov;
                    visualizer.camera.obj.updateProjectionMatrix();
                    break;
                  case "polarAngle":
                    visualizer.controls.maxPolarAngle = parseInt(
                      this.get("polarAngle").selected.split("_")[1]
                    );
                    visualizer.controls.obj.maxPolarAngle =
                      visualizer.controls.maxPolarAngle * (Math.PI / 180);
                    break;
                  case "zoomSpeed":
                    visualizer.controls.zoomSpeed = parseFloat(
                      this.get("zoomSpeed").selected.split("_")[1]
                    );
                    visualizer.controls.obj.zoomSpeed =
                      visualizer.controls.zoomSpeed;
                    break;
                  case "panSpeed":
                    visualizer.controls.panSpeed = parseFloat(
                      this.get("panSpeed").selected.split("_")[1]
                    );
                    visualizer.controls.obj.panSpeed =
                      visualizer.controls.panSpeed;
                    break;
                  case "gridHelper":
                  case "colorGrid":
                  case "colorCenter":
                  case "divisions":
                  case "gridSize":
                    visualizer.gridHelper.visible =
                      this.get("gridHelper").checked;
                    visualizer.gridHelper.colorGrid =
                      this.get("colorGrid").color;
                    visualizer.gridHelper.colorCenter =
                      this.get("colorCenter").color;
                    visualizer.gridHelper.divisions = parseInt(
                      this.get("divisions").selected.split("_")[1]
                    );
    ...
    function fnHandleSelectionOrSearch(event) {
      ...
      var grid2 =
        grid === dataVisual.dataGrid
          ? dataVisual.visualGrid
          : dataVisual.dataGrid;

      var recIds = event.searchData
        ? grid.last.searchIds // http://w2ui.com/web/docs/1.5/w2grid.last
        : grid.getSelection(true); // http://w2ui.com/web/docs/1.5/w2grid.getSelection
      if (recIds.length > 0) {
        dataVisual.layoutData.forEach(function (layoutRow) {
          layoutRow.visualObj.visible = false;
        ...
        recIds.forEach(function (recid) {
          ...
          var layoutRow = dataVisual.layoutData.find((row) => {
    ...
    function fnGridSelectColumn(dataVisual) {
      //Used by data and warehouse grid to de-select Column (from either grid) and select on current grid

      var columnIndex = dataVisual.activeGridColumn.grid.getColumn(
        dataVisual.activeGridColumn.column,
        true
      );
      for (
        var i = 0;
        i < dataVisual.activeGridColumn.grid.columns.length;
        i++
      ) {
        dataVisual.activeGridColumn.grid.columns[i].style =
          i === columnIndex
    ...
    function fnBuildScaleSlider(scaleTd, sliderTd, dataVisual) {
      ...
      slider.addEventListener("input", function () {
        ...
        dataVisual.selectedScale.scaleIndex = Math.min(
          arrayLength - 1,
          (sliderValue - dataVisual.selectedScale.direction) / 2
        );
      ...
      slider.addEventListener("change", function (event) {
        ...
        setTimeout(function () {
          fnVisualize(dataVisual);
        }, 1); //Needs slight delay for wtutils spinner to initiate.
      }); //addEventListener
    } //fnBuildScaleSlider

    function fnSetVisualScalingTitle(dataVisual) {
      var column = dataVisual.activeGridColumn.column;
      var originalColumn = fnIsPresetColor(column)
        ? column.split(dataVisual.colorPrefix)[1].trim()
        : "";
      var isKey =
        column == dataVisual.dataKey || column.trim() == dataVisual.visualKey;
      ...
      var columnText =
        column == "recid" || isKey
          ...
          : fnIsPresetColor(column)
          ? ""
          : fnIsPresetColor(dataVisual.colorPrefix + column)
          ? ": " + column
          : ": " + originalColumn + " " + text;

      w2ui[dataVisual.warehouseName + "_scaleControlLayout"].set("left", {
        title: "Visualization Scale" + columnText,
      });
      w2ui[dataVisual.warehouseName + "_scaleControlLayout"].refresh();
    } //fnSetVisualScalingTitle

    function fnSetVisualColors(dataVisual) {
      // fnSetVisualColors() with no parms resets visualizations

      if (dataVisual) {
        var column = dataVisual.activeGridColumn.column;

        if (fnIsPresetColor(column)) {
          //Preset Color
          ...
          dataVisual.join.forEach(function (joinRow, rowIndex) {
            dataVisual.setColorVisualObj(
              joinRow.visualObj,
              fnCalcColor(rowIndex, column)
            );
          }); //forEach
          ...
          var fnCalcColor = function (i, uniqueCat) {
            ...
            var rowIndex = dataVisual.join.findIndex(function (joinRow) {
              return joinRow.dataRow[originalColumn] == uniqueCat;
            });
        ...
        var columnText0 =
          column == "recid" || column == dataVisual.dataKey
            ? ""
            : colorColumn.trim() + " ";
        var text = dataVisual.activeGridColumn.grid.columns.find(function (
          col
        ) {
          return col.field == column;
        }).caption;
        ...
        var columnText =
          fnIsPresetColor(column) ||
          dataVisual.data[0][dataVisual.colorPrefix + column] != undefined
            ? columnText0 + text
            : text;
        ...
        dataVisual.join.forEach(function (joinRow) {
          joinRow.visualObj.traverse(function (node) {
            //Color the object and any children; object may be a group with children
      ...
      function fnBuildLegend(
        ...
        var legendContainer2 = d3
          .select(dataVisual.legendContainer)
          .style("background-color", "white")
          .style("height", "100%")
          .style("width", "100%")
          .style("overflow", "auto");
        ...
        var svgLegend2 = legendContainer2
          .append("svg")
        ...
        var gLegend2 = svgLegend2
          ...
          .attr(
            "class",
            dataType == "float" ? "legendSequential" : "legendOrdinal"
          )
        ...
        var aUniqueCategories =
          dataVisual.dataTypes[colorColumn].aUniqueCategories; //colorColumn is the column that is being colorized
        ...
        if (dataType == "float") {
          ...
          var ordinal = d3
            ...
            .range(
              aUniqueCategories.map(function (cat, i) {
                return colorScaleObj.fnCalcColor(i, cat);
              })
            ); //sequentialScale assigned to Ordinal logic
          legend
            .cellFilter(function (d, i) {
              return i < aUniqueCategories.length;
            })
        ...
        svgLegend2.attr(
          "width",
          Math.max(
            svgLegend2.attr("width") - 20,
            gLegend2.node().getBBox().width + 20
          )
        );
        svgLegend2.attr(
          "height",
          Math.max(
            svgLegend2.attr("height") - 20,
            gLegend2.node().getBBox().height + 20
          )
        );
      } //fnBuildLegend
    } //fnSetVisualColors
    ...
    function fnGetObjectScaleColorAndOptionallyVisualize(
      dataVisual,
      column,
      rowIndex,
      bColorVisual,
      ...
      if (colorScaleObj.minScale == colorScaleObj.maxScale) {
        ...
        var columnValue =
          ["date", "time", "dateTime"].indexOf(
            dataVisual.dataTypes[column].type
          ) != -1
            ? fnFloatFormat(dataVisual, column, data[rowIndex][column])
            : data[rowIndex][column];
        var strColorRGB =
          dataVisual.dataTypes[column].type == "float"
            ? colorScale(columnValue)
            : colorScale(
                dataVisual.dataTypes[column].aUniqueCategories.indexOf(
                  columnValue
                )
              );
      } //else
      ...
      if (bColorVisual) {
        var visualObj = dataVisual.visual.scene.getObjectByName(
          data[rowIndex][dataVisual.dataKey]
        ); //dataKey is same for inventory and warehouse
    ...
    function fnGetColumnAttributes(column, dataVisual, data) {
      var dataType = dataVisual.dataTypes[column]
        ? dataVisual.dataTypes[column].type
        : null;
    ...
    function fnGetColorScale(column, dataVisual, data) {
      ...
      var aMinMaxSequential = [
        minScale,
        Math.max(parseFloat(minScale) + 1, maxScale),
      ];
      var domain = [
        aMinMaxSequential[scaleDirection % 2],
        aMinMaxSequential[((-1 * scaleDirection) % 2) + 1],
      ]; //Flipping the domain reverses the scale [min,max] or [max,min]
  ...
  function fnFloatFormat(dataVisual, fieldValueColumn, fieldValue) {
    switch (true) {
      ...
      case dataVisual.dataTypes[fieldValueColumn].type == "dateTime":
        return w2utils.formatDateTime(
          fieldValue,
          w2utils.settings.datetimeFormat
        );
        break;
...
function fnBuildPredefinedScales(canvasParent, selectedScale) {
  ...
  var triangleHeight = 0.9 * canvas.height();
  ...
  var fnColor = d3
    .scaleSequential(d3[scale])
    .domain([
      canvasWidth * (direction % 2),
      canvasWidth * (((-1 * direction) % 2) + 1),
    ]); //direction determines scale rendering
...
function fnGetAdjustedCanvas(canvasParent) {
  ...
  var canvasWidth = parseInt(0.4 * parentWidth);
  var canvasHeight = parseInt(0.7 * parentHeight);
...
function fnGetD3Scales() {
  ...
  Object.keys(d3)
    ...
    .filter(
      (k) =>
        d3[k].prototype.constructor.toString().split("{")[0].split(",")
          .length == 1
    ) //function(t){...}
    .forEach((k) => {
      try {
...
function dataVisual() {
  ...
  this.joinDataToVisual = function (
    data,
    visual,
    dataKey,
    visualKey,
    ...
    //Paying customers can override default color and date prefix
    ...
    if (arguments.length == 1) {
      //Visual has embedded data. Visual must have mesh.name property and mesh.userData with  .userData[dataProperties]
      ...
      that.visual.scene.traverse(function (node) {
        ...
        if (node.hasOwnProperty(visualKey) && (groupTest || meshTest)) {
          ...
          if (Object.keys(node.userData).length > 0) {
            ...
            Object.keys(node.userData).forEach(function (key) {
              ...
              dataRow[key] =
                typeof node.userData[key] == "string"
                  ? d3.autoType([node.userData[key]])[0]
                  : node.userData[key];
            }); //forEach
      ...
      data.forEach(function (dataRow, i) {
        ...
        visual.scene.traverse(function (node) {
          // visuaObj may have several children deep into the hierarchy

          if (!mesh) {
            // Only continue on if the mesh hasn't been found yet
            if (node.hasOwnProperty(visualKey)) {
              // Is it a messh attribute?
    ...
    function fnSetVisualProps(dataRow, i, mesh, visualKey) {
      ...
      dataRow["recid"] = i + 1;
      dataRow[" " + visualKey] = mesh[visualKey]
        ? mesh[visualKey]
        : mesh.userData[visualKey]; //It's either in the parent or userData level

      dataRow[colorPrefix + " " + visualKey] =
        "#" + firstObjWithMaterial.material.color.getHexString();
  ...
  this.getJoinByKey = function (key, protoString) {
  ...
  this.getJoinByProperty = function (property, key, protoString) {
    // Helper function to get the joined dataRow
    // (or index, protoString == "index")
